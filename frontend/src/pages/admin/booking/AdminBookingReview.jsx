...
const AdminBookingReview = () => {
    ...
    useEffect(() => {
        const fetchReviewDetails = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:8081/api/admin/bookings/${id}/review`, {
    ...
    const handleAction = async (actionType) => {
        if (actionType === 'REJECT' && !adminReason.trim()) {
            alert("A reason must be provided when rejecting a booking.");
            return;
        }
        ...
        try {
            const token = localStorage.getItem('token');
            // Assuming the admin's email or ID from context, fallback to "Admin"
            // For production, extract this from the decoded JWT token
            const adminId = "SystemAdmin"; 
            
            const payload = {
                adminDecisionReason: adminReason || "Approved standard request",
                ...(actionType === 'APPROVE' ? { approvedBy: adminId } : { rejectedBy: adminId })
            };

            const endpoint = actionType === 'APPROVE' ? 'approve' : 'reject';
            
            const response = await fetch(`http://localhost:8081/api/admin/bookings/${id}/${endpoint}`, {
                method: 'PATCH',
                headers: {
    ...
    return (
        <DashboardLayout title="Review Booking" hideHeader hideSidebar noPadding>
            <div className="min-h-screen bg-transparent py-24 px-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-96 h-96 rounded-full bg-indigo-900/30 blur-3xl mix-blend-screen pointer-events-none"></div>
                ...
                <div className="max-w-4xl mx-auto relative z-10">
                    ...
                    <h1 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter mb-10 drop-shadow-md">
                        Review Protocol: <span className="text-indigo-400">{bookingDetails.bookingCode}</span>
                    </h1>

                    <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-2xl p-8 shadow-2xl mb-8">
                        <h2 className="text-lg font-bold text-gray-300 uppercase tracking-widest mb-6 border-b border-white/10 pb-4">Request Profile</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
                            ...
                            <div>
                                ...
                                <p className="text-lg">{bookingDetails.bookingDate} | {bookingDetails.startTime.substring(0,5)} - {bookingDetails.endTime.substring(0,5)}</p>
                            </div>
                            <div>
                    ...
                    {overlappingBookings && overlappingBookings.length > 0 && (
                        <div className="bg-rose-500/10 border border-rose-500/30 rounded-2xl p-6 shadow-2xl mb-8">
                            ...
                            <p className="text-rose-200 text-sm mb-4">The following approved bookings overlap with this time slot:</p>
                            <ul className="list-disc pl-6 text-rose-300 text-sm font-mono space-y-1">
                                {overlappingBookings.map(ob => (
                                    <li key={ob.bookingCode}>
                                        {ob.bookingCode} | {ob.startTime.substring(0,5)} - {ob.endTime.substring(0,5)}
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    {bookingDetails.status === 'PENDING' ? (
                        <div className="bg-indigo-900/20 backdrop-blur-xl border border-indigo-500/30 rounded-2xl p-8 shadow-2xl">
                            <h3 className="text-lg font-bold text-indigo-300 uppercase tracking-widest mb-6 border-b border-indigo-500/20 pb-4">Admin Decision</h3>
                            
                            <div className="mb-6">
                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Remarks / Justification</label>
                                <textarea 
                            ...
                            <div className="flex gap-4">
                                <button 
                                    ...
                                    className={`flex-1 flex justify-center items-center gap-2 py-4 rounded-xl font-black uppercase tracking-widest text-xs transition-all ${canApprove ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-900/40' : 'bg-gray-700 text-gray-500 cursor-not-allowed'}`}
                                >
                            ...
                            {!canApprove && (
                                <p className="text-rose-400 text-xs text-center mt-4">Approval disabled due to scheduling conflicts.</p>
                            )}
                        </div>
                    ) : (
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center text-gray-400">
                            This booking has already been processed and is currently marked as <span className="font-bold text-white">{bookingDetails.status}</span>.
                        </div>
                    )}
                </div>
            </div>
