/**
 * AdminBookingScanner.jsx — QR Check-In Scanner Page
 *
 * Thin orchestration page.
 * Verification API call delegated to bookingService.
...
import { verifyQrToken, getAdminIdFromToken } from '../../../services/bookingService';
...
export default function AdminBookingScanner() {
    ...
    useEffect(() => {
        ...
        const onScanSuccess = async (decodedText) => {
            ...
            // Strip the "CHECKIN_TOKEN:" prefix if present
            const tokenToVerify = decodedText.startsWith('CHECKIN_TOKEN:')
                ? decodedText.split('CHECKIN_TOKEN:')[1]
                : decodedText;

            const authToken = localStorage.getItem('token');
            const adminId = getAdminIdFromToken(); // extracted from JWT, not hardcoded

            const { data, error } = await verifyQrToken(tokenToVerify, adminId, authToken);
            setScanResult(
                data ?? { successful: false, message: error ?? 'Verification failed.' }
            );
            setVerifying(false);
        };

        // Suppress ongoing frame-scan errors from the QR library
        const onScanFailure = () => {};
    ...
    return (
        <DashboardLayout title="Access Scanner" hideHeader hideSidebar noPadding>
            <div className="min-h-screen bg-[#0a0f1c] py-24 px-6 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-[500px] h-[500px] rounded-full bg-emerald-900/20 blur-3xl mix-blend-screen pointer-events-none" />
                ...
                <div className="max-w-3xl mx-auto relative z-10">
                    ...
                    <div className="text-center mb-10">
                        ...
                        <p className="text-gray-400 text-lg">
                            Scan student QR codes to verify approval and check them into the facility.
                        </p>
                    </div>
                    ...
                    <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-2xl relative">
                        ...
                        {scannerActive && (
                            <div className="rounded-xl overflow-hidden bg-black/50 border-2 border-dashed border-white/20">
                                ...
                                {/* Override html5-qrcode default styles to match our design */}
                                <style>{`
                        ...
                        {verifying && (
                            <div className="py-20 flex flex-col items-center justify-center">
                                <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-t-2 border-emerald-400 mb-6" />
