...
const DashboardLayout = ({ children, title, noPadding = false, hideBranding = false, hideHeader = false, hideSidebar = false, hideTitle = false }) => {
    ...
    // 2. deside sidebar according to the role
    const getSidebarLinks = () => {
        ...
        if (userRole === 'ROLE_ADMIN') {
            ...
            { name: 'Availability Calendar', path: '/resources/availability', icon: <FaCalendarAlt /> },
            { name: 'Student Notifications', path: '/student/notifications', icon: <FaBell /> }, 
            ...
            { name: 'Technician Notifications', path: '/technician/notifications', icon: <FaBell /> }, 
    ...
    // Sidebar Content Component for Mobile/ Desktop
                ...
                {links.map((link, index) => {
                    ...
                    return (
                        <Link
                            key={index}
                            to={link.path}
                            onClick={() => setIsSidebarOpen(false)} //close the menu
    ...
    return (
        ...
        <div className="flex bg-gray-50 flex-1 w-full min-h-[calc(100vh-4rem)] items-stretch">
            ...
            {!hideSidebar && isSidebarOpen && (
                <div
                    className="fixed inset-0 bg-black/50 z-50 md:hidden pt-16" // 
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {!hideHeader && (
                    <header className="bg-white shadow-sm border-b border-gray-200 px-6 py-5 flex items-center gap-4">
                        {/* *sol 1 (Mobile Sidebar): Hamburger icon  */}
                        <button
